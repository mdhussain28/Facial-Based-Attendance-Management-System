...
def add_face_data_to_db(directory_path: str, conn):
    ...
    conn.create("CREATE TABLE IF NOT EXISTS image_path (id INT AUTO_INCREMENT PRIMARY KEY, name TEXT NOT NULL, image_path TEXT NOT NULL, image LONGBLOB NOT NULL)")
...
def names_table(directory_path: str, conn):
    conn.create("CREATE TABLE IF NOT EXISTS person (id INT AUTO_INCREMENT PRIMARY KEY, fname VARCHAR(30), lname VARCHAR(30))")
    files_in_directory = os.listdir(directory_path)
    image_files = [file for file in files_in_directory if file.lower().endswith(('.png', '.jpg', '.jpeg'))]
    if not image_files:
        print("No image files found in the directory!")
...
def register_no_table(directory_path: str, conn):
    conn.create("CREATE TABLE IF NOT EXISTS register_no (id INT AUTO_INCREMENT PRIMARY KEY, roll_no VARCHAR(30))")
    files_in_directory = os.listdir(directory_path)
    image_files = [file for file in files_in_directory if file.lower().endswith(('.png', '.jpg', '.jpeg'))]
    if not image_files:
        print("No image files found in the directory!")
